...
describe('Categories Controller - Unit Tests', () => {
    ...
    describe('getCategories', () => {
        it('should return all categories', async () => {
            ...
            (categoriesDal.getAllCategories as jest.Mock).mockResolvedValue(
                fakeCategories
            );
        ...
        it('should return 500 if database fails', async () => {
            (categoriesDal.getAllCategories as jest.Mock).mockRejectedValue(
                new Error('DB error')
            );
            const req = mockRequest();
            const res = mockResponse();
    ...
    describe('addCategory', () => {
        ...
        it('should return 409 if category already exists', async () => {
            (categoriesDal.upsertCategory as jest.Mock).mockResolvedValue({
                id: 1,
                name: 'Kitchen',
                inserted: false,
            });
