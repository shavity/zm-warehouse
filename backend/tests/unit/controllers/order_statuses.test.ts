...
describe('Order Statuses Controller - Unit Tests', () => {
  ...
  describe('getOrderStatuses', () => {
    it('should return all order statuses', async () => {
      ...
      (orderStatusesDal.getAllOrderStatuses as jest.Mock).mockResolvedValue(fakeOrderStatuses);
    ...
    it('should return 500 if database fails', async () => {
      (orderStatusesDal.getAllOrderStatuses as jest.Mock).mockRejectedValue(new Error('DB error'));
      const req = mockRequest();
      const res = mockResponse();
      await getOrderStatuses(req, res);
