...
describe('Rooms Controller - Unit Tests', () => {
  ...
  describe('getRooms', () => {
    ...
    it('should return 500 if database fails', async () => {
      (roomsDal.getAllRooms as jest.Mock).mockRejectedValue(new Error('DB error'));
      const req = mockRequest();
      const res = mockResponse();
      await getRooms(req, res);
  ...
  describe('addRoom', () => {
    ...
    it('should return 409 if room already exists', async () => {
      (roomsDal.upsertRoom as jest.Mock).mockResolvedValue({ id: 1, name: 'Kitchen', inserted: false });
    ...
    it('should return 201 if room created successfully', async () => {
      (roomsDal.upsertRoom as jest.Mock).mockResolvedValue({ id: 1, name: 'Kitchen', inserted: true });
      const req = mockRequest({ id: 1, name: 'Kitchen' });
      const res = mockResponse();
      await addRoom(req, res);
